...
class StartButton:
    ...
    def search_and_download(self) -> None:
        ...
        if self.query != '' and self.max_urls != '' and not self.max_urls.isalpha():
            ...
            if self.search_engine == 'Yandex':
                ...
                self.progressbar.start()
                StartButton.scrapper = ImageScrapper()
                StartButton.scrapper.scrape_yandex(query=self.query, max_urls=int(self.max_urls), url = self.url)
            if self.search_engine == 'Baidu':
                self.progressbar.place(x=40, y=122)
                self.progressbar.start()
                StartButton.scrapper = ImageScrapper()
                StartButton.scrapper.scrape_baidu(query=self.query, max_urls=int(self.max_urls), url = self.url)
            
            if self.search_engine != 'Google' and self.search_engine != 'Yandex' and self.search_engine != 'Baidu':
    ...
    def start_button(self, search_engine: str, query: str, max_urls: str, url : str, progressbar, button) -> None:
        """
...
def click_on_entry(entry) -> None:
    ...
    """
    content = entry.get()
    if content == 'Name your folder' or content == 'N' or content == "Paste your URL":
        entry.delete(0, 'end')
